class EncounterListUI
  def initialize
    ...
    if temparray.pop==7  # i picked 7 cause funny
      loctext += sprintf("<al><c2=7FF05EE8>No hay especies salvajes en esta zona</c2></al>")
     ...
     #   loctext += _INTL("<ar><c2=7FFF5EF7>{1}</c2></ar>",PBSpecies.getName(specie))
      ...
      loctext += sprintf("<al><c2=7FF05EE8>Total de especies en esta zona: %s</c2></al>",@encarray.length)
      loctext += sprintf("<c2=63184210>-----------------------------------------</c2>")
 ...
 # def pbListOfEncounters(encounter)     # old method by Vendily, got me started
 #   encable=[]
 #   return encable if !encounter
 #   for i in 0...encounter.length
 #     next if !encounter[i]
 #     for j in 0...encounter[i].length
 #       encable.push(encounter[i][j][0])
 #     end
 #   end
 #   encable|=[]
 #   return encable
...
ItemHandlers::UseFromBag.add(:POKERADAR2,proc{|item|
