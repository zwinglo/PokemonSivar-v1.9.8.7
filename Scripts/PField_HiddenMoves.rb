...
def Kernel.pbCut
  if $DEBUG ||
    ...
    if $DEBUG ||  $PokemonBag.pbQuantity(PBItems::TIJERAS)>0 #movefinder
      Kernel.pbMessage(_INTL("Hay unas espinas demasiado peligrosas.\1"))
      if Kernel.pbConfirmMessage(_INTL("¿Queres usar las tijeras?"))
        #speciesname=!movefinder ? $Trainer.name : movefinder.name
        Kernel.pbMessage(_INTL("#{$Trainer.name} ha usado tijeras de jardinería!"))
        #pbHiddenMoveAnimation(movefinder)
        return true
      end
    else
      Kernel.pbMessage(_INTL("Hay unas espinas demasiado peligrosas. No podes pasar."))
    end
  else
    Kernel.pbMessage(_INTL("Hay unas espinas demasiado peligrosas. No podes pasar."))
  end
...
def Kernel.pbSurf
  ...
  if $DEBUG || 
    ...
    #if $DEBUG || movefinder
    if $DEBUG || $PokemonBag.pbQuantity(PBItems::TABLASURF)>0
      if Kernel.pbConfirmMessage(_INTL("El agua se mira bien chiva...\n¿Queres navegar sobre ella?"))
        #speciesname=!movefinder ? $Trainer.name : movefinder.name
...
HiddenMoveHandlers::UseMove.add(:FLASH,proc{|move,pokemon|
   ...
   return false if !darkness || darkness.disposed?
     ...
     #Kernel.pbMessage(_INTL("¡{1} ha usado {2}!",pokemon.name,PBMoves.getName(move)))
   #end
   Kernel.pbMessage(_INTL("#{$Trainer.name} usa la lampara"))
