...
#        pbShowAnimation(@id,self,@battle.battlers[i],hitnum,alltargets,showanimation)
    ...
    if damage>0
      if !target.damagestate.substitute
        ...
        if target.hasWorkingAbility(:THERMALEXCHANGE) && isConst?(movetype,PBTypes,:FIRE)
          if target.pbIncreaseStatWithCause(PBStats::ATTACK,1,target,PBAbilities.getName(target.ability))
            PBDebug.log("[Ability triggered] #{target.pbThis}'s Thermal exchange")
          end
        end
        # Tragamisil
  ...
  def pbEffectsAfterHit(user,target,thismove,turneffects)
    ...
    # Battle Bond
    if isConst?(user.species,PBSpecies,:GRENINJA) && user.hasWorkingAbility(:BATTLEBOND) && $PokemonBag.pbQuantity(PBItems::MEGARING)>0
