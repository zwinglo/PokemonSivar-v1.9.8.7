...
def pbPokemonMart(stock,speech=nil,cantsell=false)
  ...
  commands[cmdQuit=commands.length]=_INTL("Solo ando viendo")
  cmd=Kernel.pbMessage(
     speech ? speech : _INTL("¿Qué va a llevar amor?"),
     commands,cmdQuit+1)
  loop do
    if cmdBuy>=0 && cmd==cmdBuy
      ...
      Kernel.pbMessage(_INTL("¡Vuelva cuando quiera!"))
      break
    end
    cmd=Kernel.pbMessage(
       _INTL("¿Puedo ayudarle en algo más?"),commands,cmdQuit+1)
  end
