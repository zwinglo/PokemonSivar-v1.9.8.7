...
class CustomTilemap
  ...
  def tileset=(value)
    if $MKXP && value.height > 8000
      parts = (value.height / 8000.0).ceil
      @divided = true
      count = 0
      for i in 0...parts
        height = count+8000 > value.height ? value.height - count : 8000
        @tilesets[i] = Bitmap.new(256,height)
        @tilesets[i].blt(0,0,value,Rect.new(0,count,256,height))
        count+=8000
      end
    end
    @tileset=value
