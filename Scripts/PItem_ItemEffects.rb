...
ItemHandlers::UseFromBag.add(:COINCASE,proc{|item|
   Kernel.pbMessage(_INTL("Bitcoins: {1}",$PokemonGlobal.coins))
   next 1 # Continue
})
...
ItemHandlers::UseOnPokemon.copy(:FIRESTONE,
   ...
   :BLACKAUGURITE,:PEATBLOCK,:LEADERSCREST,:AUSPICIOUSARMOR,:MALICIOUSARMOR)

ItemHandlers::UseOnPokemon.add(:POTION,proc{|item,pokemon,scene|
   next pbHPItem(pokemon,20,scene)
})
ItemHandlers::UseOnPokemon.add(:CARNECHUCHO,proc{|item,pokemon,scene|
   next pbHPItem(pokemon,20,scene)
})
ItemHandlers::UseOnPokemon.add(:PUPUSAS,proc{|item,pokemon,scene|
...
ItemHandlers::UseOnPokemon.add(:RARECANDY,proc{|item,pokemon,scene|
   if pokemon.level>=LevelCap.GetLimitLevel  || (pokemon.isShadow? rescue false)
     scene.pbDisplay(_INTL("No tendrá ningún efecto."))
     next false
   else
...
ItemHandlers::BattleUseOnPokemon.add(:CARNECHUCHO,proc{|item,pokemon,battler,scene|
   next pbBattleHPItem(pokemon,battler,20,scene)
})
ItemHandlers::BattleUseOnPokemon.add(:PUPUSAS,proc{|item,pokemon,battler,scene|
