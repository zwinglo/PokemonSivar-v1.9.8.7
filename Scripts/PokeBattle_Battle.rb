...
# Uses an item on an active Pokémon.
  ...
  def pbEnemyUseItem(item,battler)
    ...
    if isConst?(item,PBItems,:POTION)
      ...
      pbDisplay(_INTL("Los PS de {1} fueron restaurados.",battler.pbThis))
      elsif isConst?(item,PBItems,:CARNECHUCHO)
      battler.pbRecoverHP(20,true)
      pbDisplay(_INTL("Los PS de {1} fueron restaurados.",battler.pbThis))
      elsif isConst?(item,PBItems,:PUPUSAS)
      battler.pbRecoverHP(50,true)
      pbDisplay(_INTL("Los PS de {1} fueron restaurados.",battler.pbThis))
...
################################################################################
        ...
        if partic>0 || expshare>0 || haveexpall
          ...
          if haveexpall
            ...
            for j in 0...@party1.length
              ...
              pbDisplayPaused(_INTL("¡El resto del equipo también ha ganado experiencia!",
  ...
  def pbGainExpOne(index,defeated,partic,expshare,haveexpall,showmessages=true)
    ...
    #######################################################
    #==================CONFIGURACIÓN=======================
    levelCapExp = 1 #Experiencia que el pokémon ganara
    #recomendado 100 o menos
    
    levelCap=LevelCap.GetLimitLevel 
