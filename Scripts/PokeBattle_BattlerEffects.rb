...
#===============================================================================
  def pbCanBurn?(attacker,showMessages,move=nil)
    ...
    if self.hasWorkingAbility(:THERMALEXCHANGE) #&& isConst?(self.species,PBSpecies,:FRIGIBAX)
      @battle.pbDisplay(_INTL("Termoconversión de {1} lo protege de las quemaduras.",pbThis)) if showMessages
      return false
    end
    if self.hasWorkingAbility(:SHIELDSDOWN) &&
      isConst?(self.species,PBSpecies,:MINIOR) &&  self.form==0
  ...
  def pbCanBurnSynchronize?(opponent)
    ...
    if self.hasWorkingAbility(:THERMALEXCHANGE)#&&
      #isConst?(self.species,PBSpecies,:FRIGIBAX)
      @battle.pbDisplay(_INTL("¡{2} de {1} evitó la Sincronía!",
                              pbThis,PBAbilities.getName(self.ability)))
      return false
    end
